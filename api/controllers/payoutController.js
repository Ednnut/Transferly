...
const { createPayoutSchema, paymentTimelineQuerySchema, payoutParamsSchema } = require('../schemas/payoutSchemas');
...
async function listPayoutsController(request, response) {
  ...
  const payouts = await payoutRepository.findMany(userId ? { userId } : {});
  response.json({
